...
## 13. Decyzje zaakceptowane i uproszczenia (18.08.2026)

Odpowiedzi na pytania z sekcji 12:

1. **Stack:** PWA (React + TypeScript + Dexie). Capacitor/APK poza pierwszą wersją.
2. **Codzienne minimum:** energia, stres, rozdrażnienie, regeneracja, nastrój; jasność umysłu pod „więcej"
   (zestaw metryk konfigurowalny w Ustawieniach).
3. **Kofeina:** licznik porcji espresso (każda kawa = wielokrotność espresso); miligramy tylko do opisu
   w raporcie, przelicznik w Ustawieniach.
4. **Huawei:** preset mapowania zostanie zbudowany później, na realnym eksporcie użytkownika. Na teraz
   działa uniwersalny kreator mapowania kolumn (CSV/JSON) z podglądem przed zapisem.
5. **Język:** interfejs polski; nagłówki sekcji raportu AI pozostają angielskie (zgodnie ze specyfikacją),
   treść opisowa po polsku.
6. **Backup:** bez szyfrowania w pierwszej wersji (możliwe później).

Uproszczenia wprowadzone dla szybszego dostarczenia pierwszej wersji:

| Pierwotna propozycja | Wersja zaimplementowana | Powód |
|---|---|---|
| osobna tabela `sleep_records` z regułami precedencji źródeł | dane snu w rekordzie dnia + pole `sleepSource` | jeden rekord na dzień wystarcza; import pokazuje konflikty przed zapisem, a historia wartości jest w `revisions` |
| tabela `workouts` (0..n treningów dziennie) | jeden trening w rekordzie dnia (typ / czas / intensywność) | kilka treningów w jednym dniu to rzadki przypadek; można dodać tabelę bez migracji danych |
| `import_batches` + wycofywanie importu | podgląd przed zapisem + zapisane mapowania kolumn + `revisions` | ta sama ochrona przed błędnym importem przy znacznie mniejszej złożoności |
| Capacitor / APK w MVP | PWA instalowana z Chrome | brak potrzeby buildu natywnego; APK pozostaje ścieżką rozwoju |
| walidacja zod, szyfrowanie kopii, OCR | własna walidacja typów przy imporcie, kopia jako czysty JSON | mniej zależności; OCR i szyfrowanie jako kolejne etapy |
| React Query / store globalny | `useLiveQuery` (Dexie) jako jedyne źródło stanu | baza jest źródłem prawdy, brak duplikowania stanu |

Zrealizowany zakres pierwszej wersji: Daily Check-in z historią i edycją, sen, samopoczucie, aktywność,
styl życia, pomiary okresowe z harmonogramami, badania laboratoryjne z katalogiem i własnymi parametrami,
dashboard z zakresami 7/30/90/180/365 dni, ekran zależności (trendy, korelacje Spearmana, porównania
warunkowe, kompletność danych), import CSV/JSON z kreatorem mapowania, eksport Markdown/JSON/CSV dla AI,
kopia zapasowa i przywracanie, usunięcie wszystkich danych, ustawienia, PWA offline.

Poza pierwszą wersją (kolejne etapy): OCR wyników z PDF/zdjęcia z obowiązkową weryfikacją, preset Huawei
na realnym eksporcie, opakowanie w APK (Capacitor), szyfrowanie kopii zapasowej hasłem, adapter Health
Connect jeśli pojawi się sensowna droga danych z Huawei.
